...
class AUTH_Controller extends MY_Controller
{
	...
	public function index()
	{
		...
		else
		{
			...
			if ($this->input->get_post('search')){
				$this->addData('search',$this->input->get_post('search'));
				$this->db->like('users.full_name', $this->input->get_post('search'));
			}
	...
	public function login()
	{
		...
		else
		{
			...
			// $this->data['message'] = (validation_errors()) ? validation_errors() : $this->session->flashdata('message');
			$array[] = [
					'text' => (validation_errors()) ? validation_errors() : $this->session->flashdata('message'),
					'type' => 'info'
				];
			alert_login($array);
			...
			$this->data['identity'] = [
				...
				'id' => 'identity',
				'type' => 'text',
				'placeholder' => lang_line("login_identity_label",'auth'),
				'class' =>'form-control bg-white border-left-0 border-primary mb-2',
			...
			$this->data['password'] = [
				'name' => 'password',
				'class' =>'form-control border-left-0 border-right-0 bg-white border-primary mb-2',
				'placeholder' => lang_line("login_password_label",'auth'),
				'id' => 'password',
				'style'	=> 'border-radius:100px;',
				'type' => 'password',
			];
	...
	public function change_password()
	{
		...
		if ($this->form_validation->run() === FALSE)
		{
			...
			$this->data['old_password'] = [
				'name' => 'old',
				'id' => 'old',
				'class' =>'form-control mt-0 pt-0 border-bottom border-0 rounded-0',
				'type' => 'password',
			];
	...
	public function forgot_password()
	{
		...
		if ($this->form_validation->run() === FALSE)
		{
			...
			$this->data['identity'] = [
				'name' => 'identity',
				'id' => 'identity',
				'class' => 'form-control border-primary waves-effect'
			];
		...
		else
		{
			...
			alert($array);
				...
				$this->session->set_flashdata('message', $this->ion_auth->errors());
				redirect("profile/forgot_password", 'refresh');
			}
			...
			alert_login($array);
				...
				redirect("profile/forgot_password/code", 'refresh'); //we should display a confirmation page here instead of the login page
			}
			else
			{
	...
	public function reset_password($code = NULL)
	{
		...
		else
		{
			...
			alert($array);
			redirect("profile/forgot_password/code", 'refresh');
		}
	}
		...
		else
		{
			...
			if ($this->input->post('confirm') == 'yes')
			{
				...
				if ($this->_valid_csrf_nonce() === FALSE || $id != $this->input->post('id'))
				{
					...
					$array[] = [
						'text' => show_error($this->lang->line('error_csrf')),
						'type' => 'danger' 
					];
					alert($array);
				}
	...
	public function is_process_upgrade()
	{
		
	}
	public function generate(){
        $date = new DateTime();
        $cek = $this->ion_auth->login($this->input->post('identity',TRUE),$this->input->post('password',TRUE),false);
             
        if ($cek) {

                $payload['id'] = $_SESSION['user_id'];
                $payload["username"] = $this->input->post('identity');
